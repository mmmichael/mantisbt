...
function print_successful_redirect_to_bug( $p_bug_id , $p_bugnote_id ) {
	$t_url = string_get_bug_view_url( $p_bug_id, auth_get_current_user_id() );
	$f_bugnote_id = (int)$p_bugnote_id; 
	
	if($f_bugnote_id > 0){
		$t_url .= '#c' . $f_bugnote_id; 
	}
...
# bug_view_inc.php
function print_bug_attachments_list( $p_bug_id , $p_bugnote_id = -1, $showUeberschrift = false) {
	...
	if($showUeberschrift==true and $t_attachments_count > 0){
		echo '<br /><hr /><b>Angehängte Dateien:</b><br />'; 
	}
